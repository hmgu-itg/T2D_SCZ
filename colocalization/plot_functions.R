...
locus.zoom <- function(data = NULL, snp = NA, gene = NA, region = NA, ld.file = NULL, offset_bp = 200000, genes.data = NULL, noncoding = FALSE, plot.title = NULL, plot.type = "jpg", nominal = 6, significant = 7.3, file.name = NULL, secondary.snp = NA, secondary.label = FALSE, genes.pvalue = NULL, colour.genes = FALSE, population = "EUR", sig.type = "P", nplots = FALSE, ignore.lead = FALSE, rsid.check = TRUE, pp = NA, pp.value= NA, reg.prob=NA) {
  ...
  LD.colours <- data.frame(LD = seq(from = 0, to = 1, by = 0.1), Colour = c("#000080",rep(c("#000080", "#87CEFA", "#00FF00", "#FFA500", "#FF0000"), each = 2)), stringsAsFactors = FALSE)
  ...
  if (is.data.frame(data)) {
    ...
    if (sig.type == "P") {
      ...
      # data[logP==Inf, logP:=320]
    } else if (sig.type == "PP4"){
      data$logP = data$PP4
    } else {
    ...
    # if (sig.type == "P") {
    #   # browser()
    #   # data = lapply(data, function(x) x$logP = as.numeric(unlist(lapply(x$P, elog10))))
    #   data = lapply(data, function(x) x = x[, logP:=elog10(P)])
    #   #data[logP==Inf, logP:=320]
    # } else {
    #   data$logP = data$logBF
    # }
  ...
  if(!is.null(ld.file)){
    ...
    if (is.data.frame(data)) {
      # data.plot = merge.plot.dat(data, new.ld.file, LD.colours)
      data.plot = merge.plot.dat2(data)
    } else {
      # data.plot = lapply(data, function(x) merge.plot.dat(x, new.ld.file, LD.colours))
      data.plot = lapply(data, function(x) merge.plot.dat2(x))
    }
  }
  ...
  if(plot.type == "jpg"){
    ...
    png(width = (160 / 25.4), height = (plot.height / 25.4), filename = file.name)
  }
  ...
  if (npanel > 1) {
    for (i in 1:npanel) {
      ...
      p1 <- plot.locus(data.plot = tmp.dat, plot.title = names(data.plot)[i], secondary.snp = secondary.snp, secondary.label = secondary.label, sig.type = sig.type, plot.var = plot.var)
      rm(tmp.dat)
      # add posterior probability
      if(!is.na(pp) & !is.na(pp.value)) {
        legend(x="topleft", legend=paste0(pp," = ", pp.value), text.font=2, box.lty=0.2, cex = 1.2, inset = 0.02) #, lty=1:20) #, pt.cex = 1.2)
      }
      if(!is.na(reg.prob)){
        legend(x="topright", legend=paste0("Reg.prob = ", reg.prob), text.font=2, box.lty=0.6, cex = 1.2, inset = 0.02) #, lty=1:20) #, pt.cex = 1.2)
      }
    }
  } else {
    ...
    if(!is.na(reg.prob)){
      legend(x="topright", legend=paste0("Reg.prob = ", reg.prob), text.font=2, box.lty=0.2, cex = 1.2, inset = 0.02) #, lty=1:20) #, pt.cex = 1.2)
    }
  }
...
merge.plot.dat2 <- function(data) {
  # make all variants blue
  res <- data
  res$Colour = rep("#7F7F7F", nrow(data))
  # res$Colour = rep("#000080", nrow(data))
  return(res)
}
...
plot.secondary.point <- function(data, snp, lead.snp, plot.var, nominal, label = FALSE) {
  ...
  # plot black line around secondary SNP
  points(x = pos, y = logp, pch = 1, cex = 1.1, col = "#000000")
